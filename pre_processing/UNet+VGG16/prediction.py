...
class Train_Args:
    def __init__(self):
        self.model = "/content/drive/MyDrive/Colab Notebooks/U-Net+VGG16/checkpoints/checkpoint_epoch100.pth"
        self.input = "/content/drive/MyDrive/Colab Notebooks/U-Net+VGG16/test/Image07.jpg"
        self.output = "/content/drive/MyDrive/Colab Notebooks/U-Net+VGG16/test/Output07.jpg"
        self.viz = True
        self.no_save = False
...
def get_args():
    ...
    if args.model == "":
        args.model = "/content/drive/MyDrive/Colab Notebooks/U-Net+VGG16/checkpoints/checkpoint_epoch50.pth"
    args.input = input("Input path:").strip()
    if args.input == "":
        args.input = '/content/drive/MyDrive/Colab Notebooks/U-Net+VGG16/test/Image02.jpg'
    args.output = input("Output path:").strip()
    if args.output == "":
        args.output = '/content/drive/MyDrive/Colab Notebooks/U-Net+VGG16/test/Output02.jpg'
...
if __name__ == '__main__':
    ...
    '''#Object list in image prediction
    object_list = []
    for i in range(len(mask)):
        for j in range(len(mask[i])):
            ...
            if mask[i][j] == 1:
                ...
                class_name = "Xe bốn bánh"
            else:
                class_name = "Phương tiện khác"

            if len(object_list) == 0 and mask[i][j] >= 1:
                object_list.append([mask[i][j], class_name])
            elif len(object_list) > 0:
                for k in range(len(object_list)):
                    if mask[i][j] == object_list[k][0]:
                        continue
                    else:
                        object_list.append([mask[i][j], class_name])

    #Display segment and predict results
    display = "<*> Object predicted: "
    for i in range(len(object_list)):
        display += object_list[i][1]
