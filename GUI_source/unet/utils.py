...
class BasicDataset(Dataset):
    def __init__(self, images_dir: str, mask_dir: str, scale: float = 1.0, mask_suffix: str = '', augment: bool = False):
        ...
        if not self.ids:
            raise RuntimeError(f'Lihie_Nguyn>>> No input file found in {images_dir}, make sure you put your images there')
        ...
        if self.augment:
            self.transform = A.Compose([
                ...
                A.GaussNoise(p=0.2),
                A.ElasticTransform(alpha=50, sigma=6, alpha_affine=30, p=0.2),
            ])
        else:
            self.transform = None
